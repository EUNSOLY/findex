...
public class IntegrationApplication {
    ...
    public SyncJobResult getSyncJobs(SyncJobsCommand command, CursorRequest cursorRequest) {
        IntegrationHistorySearchCondition condition = IntegrationHistorySearchCondition.of(command.jobType(), command.indexInfoId(),
    ...
    public List<SyncIndexResult> syncIndexInfos(String clientIp) {
        List<CreateIndexInformationCommand> indexInfoCommands = indexInformationProvider.getOpenApiIndexInfos();
        ...
        for (int i = 0; i < indexInfoCommands.size(); i += CHUNK_SIZE) {
            List<CreateIndexInformationCommand> chunk = indexInfoCommands.subList(i, Math.min(i + CHUNK_SIZE, indexInfoCommands.size()));
            try {
                results.addAll(transactionTemplate.execute(status -> this.processIndexInfoChunk(chunk, clientIp, jobTime)));
            } catch (Exception e) {
    ...
    private List<SyncIndexResult> processIndexInfoChunk(List<CreateIndexInformationCommand> chunk, String clientIp, LocalDateTime jobTime) {
        List<SyncIndexResult> result = new ArrayList<>();

        for (CreateIndexInformationCommand command : chunk) {
            IndexInformation savedIndexInfo = indexInformationService.upsert(command.toEntity(CREATE_TYPE));
            IntegrationHistoryCommand historyCommand = IntegrationHistoryCommand.of(savedIndexInfo, null, clientIp, jobTime);
    ...
    private List<SyncIndexResult> processIndexInfoOneByOne(List<CreateIndexInformationCommand> chunk, String clientIp, LocalDateTime jobTime) {
        List<SyncIndexResult> result = new ArrayList<>();

        for (CreateIndexInformationCommand command : chunk) {
            try {
                result.add(transactionTemplate.execute(status -> {
                    IndexInformation saved = indexInformationService.upsert(command.toEntity(CREATE_TYPE));
                    IntegrationHistory history = IntegrationHistoryCommand.of(saved, null, clientIp, jobTime).toEntity("INDEX_INFO", true);
                    return SyncIndexResult.of(integrationHistoryService.saveSuccess(history), saved);
                }));
            } catch (Exception e) {
                log.warn("지수 정보 연동 실패: {} / {}", command.indexClassification(), command.indexName(), e);

                result.add(transactionTemplate.execute(status -> {
                    IntegrationHistory history = IntegrationHistoryCommand.of(null, null, clientIp, jobTime).toEntity("INDEX_INFO", false);

                    return SyncIndexResult.of(integrationHistoryService.saveFailed(history), null);
                }));
            }
        }
    ...
    public List<SyncIndexResult> syncIndexData(SyncIndexDataCommand command, String clientIp) {
        ...
        List<IndexInformation> indexInformationTargets = indexInformationService.findSyncTargets(command.indexInfoIds());
        List<SyncIndexResult> results = new ArrayList<>();
        ...
        for (IndexInformation indexInformation : indexInformationTargets) {
            ...
            do {
                try {
                    ...
                    page = indexInformationProvider.getOpenApiIndexData(pageNo, indexClassification, indexName, command.baseDateFrom(),
                            command.baseDateTo());
                } catch (RuntimeException e) {
                    log.warn("OpenAPI 호출 실패: {} (page {})", indexName, pageNo, e);
                    IntegrationHistory history =
                            IntegrationHistoryCommand.of(indexInformation, null, clientIp, jobTime).toEntity("INDEX_DATA", false);

                    results.add(SyncIndexResult.of(integrationHistoryService.saveFailed(history), indexInformation));
                    break; // 다음 지수로
                }
                if (!page.items().isEmpty()) {
                    results.addAll(processIndexDataChunk(indexInformation, page.items(), clientIp, jobTime)); // 트랜잭션
                }

                List<CreateIndexDataCommand> items = page.items();
                for (int i = 0; i < items.size(); i += CHUNK_SIZE) {
                    List<CreateIndexDataCommand> chunk = items.subList(i, Math.min(i + CHUNK_SIZE, items.size()));
                    results.addAll(processIndexDataChunk(indexInformation, chunk, clientIp, jobTime));
                }
    ...
    private List<SyncIndexResult> processIndexDataChunk(IndexInformation indexInformation, List<CreateIndexDataCommand> items, String clientIp,
    ...
    private List<SyncIndexResult> processIndexDataOneByOne(IndexInformation indexInformation, List<CreateIndexDataCommand> items, String clientIp,
            LocalDateTime jobTime) {
        List<SyncIndexResult> results = new ArrayList<>();
        ...
        for (CreateIndexDataCommand command : items) {
            try {
                ...
                log.warn("지수 데이터 연동 실패: {} / 기준 일자: {}", indexInformation.getIndexName(), command.baseDate(), e);
                results.add(transactionTemplate.execute(status -> {
                    IntegrationHistory history =
                            IntegrationHistoryCommand.of(indexInformation, command.baseDate(), clientIp, jobTime).toEntity("INDEX_DATA", false);
                    return SyncIndexResult.of(integrationHistoryService.saveFailed(history), indexInformation);
                }));
            }
        }
        return results;
